...
function App() {
  ...
  const filteredTask = (filter: FilterValuesType, currentData: TaskType[]) => {
    switch (filter) {
      case "active":
        return currentData.filter(item => !item.isDone)
      case "completed":
        return currentData.filter(item => item.isDone)
      default:
        return currentData
  ...
  const tasksForTodolist = filteredTask(filter, tasks)
